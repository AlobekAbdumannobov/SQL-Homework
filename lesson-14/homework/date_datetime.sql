--CREATE DATABASE SQL_Practice14
--go
--use SQL_Practice14
...
--2 Write a SQL query to find strings from a table where the string itself contains the % character.(TestPercent)
 --TestPercent jadvalidan o‘z ichida “%” belgisi mavjud bo‘lgan satrlarni toping.
...
-- Splitter jadvalida berilgan satrni nuqta (.) bo‘yicha ajrating.

SELECT 
    vals,
    LEFT(vals, CHARINDEX('.', vals + '.') - 1) AS Part1,   -- 1-qism
...
-- testDots jadvalidan Vals ustunidagi qiymatda ikki (2) tadan ortiq nuqta (.) mavjud bo‘lgan satrlarni chiqaring.
...
-- CountSpaces jadvalidan satrdagi bo‘sh joylar (space) sonini hisoblang.
...
-- Employee jadvalidan o‘z menejeridan (manager) ko‘proq maosh oladigan xodimlarni toping.
...
--Employees jadvalidan kompaniyada 10 yildan ko‘p, lekin 15 yildan kam ishlagan xodimlarni toping.
--Natijada quyidagi ustunlar chiqsin:
--Employee ID
--First Name
--Last Name
--Hire Date
--Years of Service (ya’ni hozirgi sana bilan ishga kirgan sana orasidagi yil farqi).
...
-- weather jadvalidan avvalgi (kechagi) kunga nisbatan harorati yuqori bo‘lgan sanalarning Idlarini toping.
...
-- Activity jadvalidan har bir o‘yinchi (player) uchun birinchi kirish (login) sanasini ko‘rsating.
...
-- fruits jadvalidagi ro‘yxatdan uchinchi elementni (item) qaytaring.
...
--Employees jadvalidan har bir xodimning Ish bosqichi (Employment Stage) ni HIRE_DATE asosida aniqlang:
--1 yildan kam → ‘New Hire’
--1 yildan 5 yilgacha → ‘Junior’
--5 yildan 10 yilgacha → ‘Mid-Level’
--10 yildan 20 yilgacha → ‘Senior’
--20 yildan ortiq → ‘Veteran’
SELECT 
    EMPLOYEE_ID,
    FIRST_NAME,
    LAST_NAME,
...
--GetIntegers jadvalidagi Vals ustunida satr boshida joylashgan butun son (integer) ni ajratib oling.

SELECT 
    Id,
    LEFT(VALS, PATINDEX('%[0-9]%', VALS) - 1 + 
         PATINDEX('%[^0-9]%', SUBSTRING(VALS, PATINDEX('%[0-9]%', VALS), LEN(VALS)))) AS Integer_Part
FROM GetIntegers
WHERE VALS LIKE '[0-9]%';
...
-- MultipleVals jadvalidagi vergul bilan ajratilgan satrda birinchi ikkita harfni joyini almashtiring (swap).
...
-- Berilgan satr (sdgfhsdgfhs@121313131) asosida har bir belgini (character) alohida qatorda (row) chiqadigan jadval yarating.

DECLARE @str VARCHAR(50) = 'sdgfhsdgfhs@121313131';

SELECT 
    SUBSTRING(@str, v.number, 1) AS Character
...
-- Activity jadvalidan har bir o‘yinchi (player) uchun birinchi marta tizimga kirgan (login qilgan) qurilmani (device) aniqlang.
...
-- Berilgan rtcfvty34redt satrni butun sonlar (integer values) va harflar (character values) sifatida ajrating va ularni ikkita alohida ustunga joylang.

DECLARE @str VARCHAR(50) = 'rtcfvty34redt';

SELECT 
    STRING_AGG(CASE WHEN c LIKE '[0-9]' THEN c END, '') WITHIN GROUP (ORDER BY n) AS Numbers,
    STRING_AGG(CASE WHEN c LIKE '[A-Za-z]' THEN c END, '') WITHIN GROUP (ORDER BY n) AS Letters
FROM (
    SELECT SUBSTRING(@str, n, 1) AS c, n
    FROM (SELECT TOP (LEN(@str)) ROW_NUMBER() OVER (ORDER BY (SELECT NULL)) AS n
          FROM sys.objects) t
    WHERE n <= LEN(@str)
) t;
